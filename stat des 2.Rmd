...
```
# 4. Nombre moyen d'actifs en portefeuille
...
```{r}
'''
# Charger la base de données ESG_Funds_Holdings_All
ESG_data_sector <- ESG_Funds_Holdings_All

# Fusionner les données en utilisant la colonne ISIN comme clé de fusion
merged_data <- merge(ESG_data_sector, Covalence_AllData, by.x = "isin", by.y = "ISIN", all.x = TRUE)

# Afficher les premières lignes du résultat fusionné
head(merged_data)
'''
```
