...
sim_parameters = reset.sim(time.periods = time.periods, n = 1000, ipw = TRUE, reg = TRUE)
...
ggplot(data, aes(x = period, y = Y, color = G, linetype = factor(treat))) +
  geom_point() +
...
# run event study regression
# normalize effect to be 0 in pre-treatment period
es_copie <- plm(Y ~ Dtmin3 + Dtmin2 + Dt0 + Dt1 + Dt2, 
          data = copie_dataset, model = "within", effect = "twoways",
          index = c("id", "period"))
