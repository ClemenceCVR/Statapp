...
fr_pondere_ESG <- data.frame(date = weighted_sum$date, id = weighted_sum$id, Outcome = weighted_average, category=category$Poids_Fonds_ESG)
```
...
Poids_Fonds_noESG <- noESG_Funds_Holdings_All %>%
  group_by(fund_Isin, date, category) %>%
  mutate(poids = market_value / weight) %>%
  slice(1) %>%  # Garder seulement la première ligne de chaque groupe
